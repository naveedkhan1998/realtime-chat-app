...
class CloudflareSFUService:
    ...
    def create_session_for_user(
        ...
        if not self.is_configured:
            logger.warning(
                "Cloudflare Calls SFU not configured. APP_ID set: %s, APP_SECRET set: %s",
                bool(self.app_id),
        ...
        if existing_session_id:
            logger.debug(
                "Using existing session %s for user %d in room %d",
                existing_session_id,
                user_id,
                room_id,
            )
    ...
    def add_track(
        ...
        try:
            ...
            logger.info(
                "Adding track for user %d, track: %s, session: %s",
                user_id,
                track_name,
                session_id,
            )
            logger.debug(
                "SDP offer length: %d chars, request body keys: %s",
                len(sdp_offer),
            ...
            data = response.json()
            logger.info(
                "Track added successfully, response keys: %s", list(data.keys())
            )
            ...
            if response_tracks:
                for i, track in enumerate(response_tracks):
                    ...
                    track_name_resp = track.get(
                        "trackName", f"{user_id}_{track_name}_{i}"
                    )
                    logger.debug(
                        "Track %d: mid=%s, name=%s", i, track_mid, track_name_resp
                    )
                    ...
                    self.redis.hset(
                        tracks_key, f"{user_id}_{track_name}_{i}", track_info
                    )
    ...
    def subscribe_to_tracks(
        ...
        try:
            ...
            for track_key, track_data in all_tracks.items():
                ...
                if track_info["user_id"] != user_id:
                    ...
                    if publisher_session_id and track_name:
                        ...
                        logger.debug(
                            "Subscribing to track '%s' from session %s",
                            track_name,
            ...
            if not remote_tracks:
                logger.debug(
                    "No remote tracks available for user %d to subscribe to", user_id
                )
                return None

            logger.info(
                "User %d subscribing to %d remote tracks", user_id, len(remote_tracks)
            )
            ...
            if data.get("sessionDescription"):
                logger.info(
                    "Received SFU-generated offer, type: %s, requiresRenegotiation: %s",
    ...
    def remove_user_session(self, room_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            logger.info(
                "Removed session and tracks for user %d in room %d", user_id, room_id
            )
            return True
