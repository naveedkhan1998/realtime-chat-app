...
interface ConnectionDetail {
  ...
  audio?: {
    ...
    inbound?: { packetsReceived?: number; jitter?: number; codec?: { mimeType?: string } };
...
const STATE_CONFIGS: Record<string, { icon: LucideIcon; color: string; bg: string }> = {
  ...
  'in-progress': { icon: CircleDot, color: 'text-yellow-500', bg: 'bg-yellow-500/10' },
  frozen: { icon: WifiOff, color: 'text-gray-500', bg: 'bg-gray-500/10' },
};

const DEFAULT_STATE_CONFIG = { icon: CircleDot, color: 'text-gray-500', bg: 'bg-gray-500/10' };

const BAR_HEIGHTS = ['h-1', 'h-2', 'h-3', 'h-4'] as const;

// ============ MEMOIZED SUB-COMPONENTS ============
const QualityIndicator = memo(({ quality }: { quality: string }) => {
  const config = QUALITY_CONFIGS[quality as keyof typeof QUALITY_CONFIGS] || DEFAULT_QUALITY_CONFIG;
        ...
        {[1, 2, 3, 4].map(i => (
          <div
            ...
            className={cn(
              ...
              i <= config.bars ? config.color.replace('text-', 'bg-') : 'bg-muted',
              BAR_HEIGHTS[i - 1]
            )}
          />
        ))}
...
const ConnectionPathVisual = memo(({ path, type }: { path: string; type: string }) => {
      ...
      <div className="relative flex items-center justify-between">
        ...
        <div className="relative flex-1 mx-4">
          ...
          <div className="absolute inset-0 overflow-hidden">
            <div className="absolute w-2 h-2 rounded-full top-1/2 -translate-y-1/2 bg-primary animate-[ping_2s_ease-in-out_infinite]" style={{ left: '20%' }} />
            <div className="absolute w-2 h-2 rounded-full top-1/2 -translate-y-1/2 bg-primary animate-[ping_2s_ease-in-out_infinite_0.5s]" style={{ left: '50%' }} />
            <div className="absolute w-2 h-2 rounded-full top-1/2 -translate-y-1/2 bg-primary animate-[ping_2s_ease-in-out_infinite_1s]" style={{ left: '80%' }} />
          </div>
          ...
          {!isDirect && (
            <div className="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
              <div className={cn(
                ...
                isRelay ? 'bg-orange-500/10 border-orange-500/30' : 'bg-blue-500/10 border-blue-500/30'
              )}>
                {isRelay ? (
          ...
          {isDirect && (
            <div className="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
              <div className="flex items-center gap-1 px-2 py-1 border rounded-full bg-green-500/10 border-green-500/30">
                ...
                <span className="text-[9px] font-semibold text-green-500">DIRECT</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-[10px] font-medium text-muted-foreground">Peer</span>
        </div>
      </div>
...
const BitrateChart = memo(({ inbound, outbound }: { inbound: number; outbound: number }) => {
      ...
      <div className="space-y-3">
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono font-medium">{inbound.toFixed(1)} kbps</span>
          </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono font-medium">{outbound.toFixed(1)} kbps</span>
          </div>
          <div className="relative h-2 overflow-hidden rounded-full bg-muted">
            <div
...
}, (prev, next) => prev.inbound === next.inbound && prev.outbound === next.outbound);
BitrateChart.displayName = 'BitrateChart';

const CandidatePairCard = memo(({ pair, isActive }: { pair: CandidatePair; isActive: boolean }) => {
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className={cn('text-xs font-medium uppercase', stateConfig.color)}>
            {pair.state}
          </span>
        </div>
        {isActive && (
...
const PeerStats = memo(({ peer, details, showHeader }: PeerStatsProps) => {
  ...
  const filteredPairs = useMemo(() => {
    ...
    return details.candidatePairs
      .filter(pair => pair.state === 'succeeded' || pair.state === 'failed' || pair.selected)
      .sort((a, b) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 space-y-2 border rounded-lg bg-muted/10">
        <h4 className="text-xs font-medium text-muted-foreground">Connection Details</h4>
        <div className="grid grid-cols-2 text-xs gap-x-4 gap-y-1">
          ...
          {details.audio?.inbound?.codec && (
            <>
              ...
              <span className="font-mono">
                {details.audio.inbound.codec.mimeType?.split('/')[1]?.toUpperCase() || 'N/A'}
              </span>
            </>
          )}
        </div>
...
function WebRTCStats({
  ...
  const emptyState = useMemo(() => (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="font-medium text-muted-foreground">No active connections</p>
